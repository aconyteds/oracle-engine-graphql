...
test("Unit -> analyzeConversationContext analyzes agent performance", async () => {
  const messages = [
    ...
    createMessage("5", "The answer is 4", "assistant", "2024-01-01T10:04:00Z", {
      decision: {
  ...
  expect(parsed.agentPerformance["Character Generator"].successRate).toBe(1.0);
  expect(parsed.agentPerformance["Character Generator"].userSatisfaction).toBe(
    "positive"
  );
...
test("Unit -> analyzeConversationContext detects workflow continuation pattern", async () => {
  const messages = [
    createMessage("1", "Create a character", "user", "2024-01-01T10:00:00Z"),
    createMessage("2", "Set character stats", "user", "2024-01-01T10:01:00Z"),
    createMessage("3", "Choose character race", "user", "2024-01-01T10:02:00Z"),
    createMessage(
      "4",
...
test("Unit -> analyzeConversationContext assesses knowledge buildup", async () => {
  ...
  expect(["medium", "high"]).toContain(knowledgeFactor?.contextValue);
});
